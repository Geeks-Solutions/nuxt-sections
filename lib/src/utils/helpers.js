...
export const renderPageData = async (app, pageName) => {
  ...
  try {
    ...
    if (hooksJavascript['init_params']) {
      const paramsUpdate = hooksJavascript['init_params'](app.context.$sections, { qs: app.context.route.query, headers: app.context && app.context.req ? app.context.req.headers : {}, reqBody: app.context && app.context.req ? app.context.req.body : {} })
      if (paramsUpdate) {
