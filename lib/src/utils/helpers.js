...
export const renderPageData = async (app) => {
  let websiteDomain = ""
  ...
  let URL = `${app.context.$sections.serverUrl}/project/${app.context.$sections.projectId}/page/${parsePath(encodeURIComponent(app.context.route.params.pathMatch ? app.context.route.params.pathMatch : '/'))}`;

  if (app.context.$sections.cname === "active") {
    URL = `${app.context.$sections.serverUrl}/project/${websiteDomain}/page/${parsePath(encodeURIComponent(app.context.route.params.pathMatch ? app.context.route.params.pathMatch : '/'))}`;
  }
  let payload = {}

  let language = undefined
  try {
    if (app.context.i18n.locale !== app.context.i18n.defaultLocale) {
