<template>
  <div>
    <div class="input mt-8">
      ...
      />
      <span
        v-if="errors.quill === true && selectedLang === defaultLang"
        class="flexSections sections-required-field-error"
        >{{ t('requiredField') }}</span
      >
      <span
        v-else-if="errors.quill === true && selectedLang !== defaultLang"
        id="required-fields"
        class="flexSections sections-required-field-error"
...
watch(
  () => settings.value,
  (newVal) => {
    // Example if using initialSettings prop
...
) // Run immediately and watch deeply
...
const validate = () => {
  ...
  if (Array.isArray(settings.value)) {
    for (let i = settings.value.length - 1; i >= 0; i--) {
      ...
      for (const lang of props.locales) {
        if (typeof settings.value[0][lang] === 'string') {
          ...
          imgTags.forEach((img) => {
            ...
            const foundM = settings.value.find(
              (m) =>
  ...
  if (
    !settings.value[0][props.defaultLang] ||
    settings.value[0][props.defaultLang] === '<p><br></p>'
  ) {
