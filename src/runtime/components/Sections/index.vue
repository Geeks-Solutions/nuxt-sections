      ...
      <div class="sections-config sections-justify-center">
                        ...
                        >
                          <div class="section-view relativeSections">
                            <div class="view-component" :style="{ background: viewsBgColor }">
                              <component
                                v-if="
                                  view.settings || view.type === 'local' || view.type === 'dynamic'
                                "
                                :is="getComponent(view.name, view.type, view)"
                                :section="view"
                                :lang="lang"
        ...
        <div v-else>
          ...
          >
            <component
              :is="
                registeredPage(errorResponseStatus === 404 ? 'page_not_found' : 'project_not_found')
              "
...
const { pageName, admin, variations, lang, viewsBgColor, sectionsPageData } = defineProps({
  pageName: {
    type: String,
    default: '',
  },
...
onMounted(async () => {
  ...
  if (
    (errorResponseStatus.value === 429 && sectionsError.value !== '') ||
    (sectionsError.value !== '' &&
      !registeredPage(errorResponseStatus.value === 404 ? 'page_not_found' : 'project_not_found'))
  ) {
    showToast(
      'Error',
      'error',
      i18n.t('loadPageError') + sectionsError.value,
      sectionsErrorOptions.value
    )
  }
