    ...
    <div class="sections-container sections-container-edit-mode">
      <aside
        v-if="
          (admin &&
            editMode &&
            isSideBarOpen === true &&
            (currentSection !== null || metadataModal === true || sectionsThemeModal === true)) ||
          (admin && isSideBarOpen === true && dynamicSideComponent === true)
        "
        ref="resizeTarget"
      ...
      >
        ...
        <a
          ...
          class="anchorIcon"
          :href="
            currentSection.linked_to !== '' && currentSection.linked_to !== undefined
              ? `#${currentSection.linked_to}-${currentSection.id}`
              : `#${currentSection.name}-${currentSection.id}`
          "
        >
          <LazyBaseIconsAnchor
            :title="
              currentSection.linked_to !== '' && currentSection.linked_to !== undefined
                ? `Anchor id: #${currentSection.linked_to}-${currentSection.id}`
                : `Anchor id: #${currentSection.name}-${currentSection.id}`
            "
            class="edit-icon"
          />
        </a>
        <div
          v-if="currentSection"
          :ref="currentSection.name === 'SimpleCTA' ? 'intro-simple-CTA-section-form' : undefined"
          :class="currentSection.name === 'SimpleCTA' ? 'intro-simple-CTA-section-form' : ''"
          class="flexSections component-view-wrapper"
        >
        ...
        <div v-if="!currentSection && metadataModal === true" class="section-modal-wrapper">
          ...
          <LazyTranslationsTranslationComponent
            v-if="
              ...
              locales.length > 1 &&
              currentSettingsTab === 'page_settings'
            "
            :locales="locales"
            :default-lang="defaultLang"
          ...
          />
          <div
            v-if="currentSettingsTab === 'page_settings'"
              ...
              <div class="flexSections sections-flex-row sections-gap-4 metadataFieldsContainerRow">
                <div class="sections-w-full sectionsFieldsLabelsWrapper">
                  ...
                  <input
                    ...
                    v-model="pagePath"
                  />
                  <span class="pagePathRequiredStyle" v-show="metadataErrors.path[0] !== ''">{{
                ...
                <div>
                  <div>
                    ...
                    <LazyMediasUploadMedia
                      :media-label="''"
                      :upload-text="$t('mediaComponent.Upload')"
                      :change-text="$t('mediaComponent.Change')"
                      :seo-tag="$t('mediaComponent.seoTag')"
                      :media="
                  ...
                  <div>
                    ...
                    <LazyMediasMediaComponent
                      ref="sectionsMediaComponent"
                      :sections-user-id="sectionsUserId"
                      @emittedMedia="(mediaObject) => selectedCSS(mediaObject, selectedMediaType)"
                    ></LazyMediasMediaComponent>
                  </div>
                </div>
              </div>
        ...
        <div
          v-if="!currentSection && sectionsThemeModal === true"
          class="section-modal-wrapper sections-themes"
        >
          <div
            v-if="sectionsThemeComponents[currentSectionData.section.name].length > 1"
            class="sections-theme-settings-tabs"
          >
            <div
              v-for="tab in sectionsThemeComponents[currentSectionData.section.name]"
            ...
            >
              ...
              >
                <div class="section-info-icon">
                  <LazyBaseIconsAlert
                    :title="$t('sectionsSettings.save_page_settings')"
                    color="#f59e0b"
          ...
          >
            <LazyBaseIconsAnchor
              :title="
                currentSectionData.section.linked_to !== '' &&
                currentSectionData.section.linked_to !== undefined
                  ? `Anchor id: #${currentSectionData.section.linked_to}-${currentSectionData.section.id}`
                  : `Anchor id: #${currentSectionData.section.name}-${currentSectionData.section.id}`
              "
              class="edit-icon"
            />
          </div>
          ...
          <div class="section-theme-component-wrapper">
            <component
              :is="getDynamicComponent(currentThemeTab.path)"
              ...
              @section-theme-updated="sectionThemeUpdated"
            ></component>
            <div class="footer">
      ...
      <main ref="sectionsMainTarget" class="sections-main">
        <div class="sections-config sections-justify-center">
          <div v-if="!pageNotFound">
            ...
            <div
              class="bg-light-grey-hp hide-mobile section-wrapper config-wrapper"
              ref="sectionWrapper"
            >
              <div
              ...
              >
                <div class="flexSections sections-flex-row sections-justify-center top-bar-wrapper">
                  <div class="flexSections top-bar-page-content-wrapper">
                    ...
                    <button class="hp-button" type="button" @click="isCreatePageModalOpen = true">
                      <div class="plus-icon">
                        <LazyBaseIconsPlus />
                      </div>
                    </button>
                    ...
                    <LazyBaseHelperComponentsBurgerMenu>
                      <template #content>
                        <button
                          ...
                          data-toggle="tooltip"
                          data-placement="top"
                          :title="$t('importSectionsLabel')"
                          @click="initImportSections"
                        >
                          <LazyBaseIconsExport />
                          <span class="sections-pl-2 sections-no-wrap">{{
                            $t('importSectionsLabel')
                          }}</span>
                        </button>
                        <input
                          ref="jsonFilePick"
                          type="file"
                          @change="(e) => importSections(e)"
                          style="display: none"
                        />
                        <button
                          class="hp-button"
                          ...
                          data-toggle="tooltip"
                          data-placement="top"
                          :title="$t('exportSectionsLabel')"
                          @click="exportSections"
                        >
                          <LazyBaseIconsImport />
                          <span class="sections-pl-2 sections-no-wrap">{{
                            $t('exportSectionsLabel')
                          }}</span>
                        </button>
                        ...
                        <div class="flexSections sections-flex-col">
                          ...
                          >
                            <div class="btn-text">
                              {{ layoutMode === true ? $t('hideLayout') : $t('editLayout') }}
                            </div>
                          </button>
                          <div
                            v-if="layoutMode === true"
                            class="flexSections sections-flex-row sections-gap-4 layout-region-wrapper"
                          >
                  ...
                  <div
                    ref="intro-top-bar"
                    class="intro-top-bar flexSections top-bar-page-content-wrapper"
                  >
                    <div
                      :ref="selectedLayout === 'standard' ? 'intro-add-new-section' : ''"
                      :class="selectedLayout === 'standard' ? 'intro-add-new-section' : ''"
                    >
                      <button
                    ...
                    <button
                      ref="intro-save-changes"
                      class="intro-save-changes hp-button"
                  ...
                  <div
                    v-show="sectionsOptionsComponentPath !== ''"
                    class="sections-config-separator"
                  ></div>
                  ...
                  <div class="flexSections top-bar-page-content-wrapper">
                    ...
                    <LazyBaseHelperComponentsBurgerMenu class="settings">
                      ...
                      <template #content>
                        <button
                          v-for="tab in updatedPageSettingsTabs
                            .filter((tab) => tab !== 'page_settings')
                            .reverse()"
            ...
            <div
              v-if="isModalOpen && admin && editMode"
              ref="modal"
            ...
            >
              ...
              >
                <div
                  class="section-modal-content sections-bg-white relativeSections sections-shadow rounded-xl"
                >
                  <div
                    class="flexSections sections-flex-row relativeSections sections-justify-center"
                  >
                    <div
                      v-if="!currentSection && isCreateInstance === false"
                      class="flexSections sections-flex-col sections-my-3 sections-gap-4"
                    >
                      <div
                        class="flexSections sections-items-center sections-flex-row sections-gap-4 section-types-filter"
                      >
                        <div>{{ $t('filterBy') }}</div>
                        <input
                        ...
                        <div v-if="typesTab !== 'inventoryTypes'" class="relativeSections">
                          ...
                          >
                            ...
                            <option
                              v-for="appName in appNames.filter(
                                (item, index) => appNames.indexOf(item) === index
                              )"
                              :value="appName"
                            >
                              {{ appName }}
                        ...
                        <div
                          class="slot-name sections-cursor-pointer"
                          @click="clearSectionsFilters"
                        >
                          {{ $t('filterClear') }}
                        </div>
                      </div>
                    ...
                    <div
                      ...
                      class="flexSections sections-flex-row sections-my-3 sections-pb-6 sections-justify-center"
                    >
                      <div
                        class="sections-text-center h2 sections-cursor-pointer selectSectionType"
                      >
                        {{ $t('selectSectionType') }}
                      </div>
                    </div>
                  ...
                  <div class="step-back" v-if="currentSection" @click="currentSection = null">
                    <LazyBaseIconsBack />
                  </div>
                  ...
                  <div
                    v-if="
                      !currentSection &&
                      (typesTab === 'types' || typesTab === 'inventoryTypes') &&
                      isCreateInstance !== true &&
                      filteredGlobalTypes.filter((gt) => gt.id !== undefined)?.length > 0
                    "
                    class="flexSections sections-flex-row my-global-content"
                    @click="showMyGlobal = !showMyGlobal"
                  >
                  ...
                  <div
                    v-if="
                      ...
                      isCreateInstance !== true
                    "
                    ref="intro-available-sections"
                    class="intro-available-sections sections-m-1 sections-p-1 type-items content-wrapper"
                  >
                    <div
                      v-for="(type, index) in [
                        ...(showMyGlobal
                          ? filteredGlobalTypes.filter((gt) => gt.id !== undefined)
                          : []),
                        ...filteredTypes.filter(
                          (type) =>
                            type.notCreated !== true &&
                            type.app_status !== 'disbaled' &&
                      ...
                      :class="[
                        type.id === 'addition-empty' ? 'addition-empty' : '',
                        showMyGlobal &&
                        index < filteredGlobalTypes.filter((gt) => gt.id !== undefined).length
                          ? 'global'
                          : '',
                      ]"
                    >
                      <div
                        v-if="
                          type.type === 'local' ||
                          componentsSetupData(
                            type.section?.name || type.name,
                            type.type ? type.type : 'static'
                          ).settings ||
                          componentsSetupData(type.section?.name || type.name, type.type)
                            .render_data
                        "
                        :title="formatTexts(formatName(type.name), ' ')"
                        class="text-capitalize section-item-title"
                      >
                        {{ formatTexts(formatName(type.name), ' ') }}
                      </div>
                      <div
                        v-if="
                          (type.access === 'private' && type.notCreated !== true) ||
                          (type.notCreated !== true && type.section)
                        "
                      ...
                      >
                        <div
                          ...
                          @click="
                            openDeleteSectionTypeModal(
                              type.name,
                              index,
                              showMyGlobal &&
                                index <
                                  filteredGlobalTypes.filter((gt) => gt.id !== undefined).length
                            )
                          "
                        >
                          <LazyBaseIconsTrash class="trash-icon-style" />
                      ...
                      <div
                        v-else-if="type.query_string_keys && type.query_string_keys.length > 0"
                        class="section-info"
                      >
                        <LazyTooltipClickableTooltip
                          :content="`query_string(s): ${type.query_string_keys.join(', ')}`"
                          position="top"
                        >
                          <div class="section-info-icon">
                            <LazyBaseIconsInfo
                              :title="`query_string(s): ${type.query_string_keys.join(', ')}`"
                              class="info-icon-style"
                            />
                          </div>
                        </LazyTooltipClickableTooltip>
                        ...
                        @click="
                          ...
                          index < filteredGlobalTypes.filter((gt) => gt.id !== undefined).length
                            ...
                            : type.notCreated !== true
                              ...
                              : openCurrentSection(type, false, true)
                        "
                      >
                      ...
                      <div
                        v-if="
                          ...
                          type.notCreated !== true
                        "
                        class="flexSections sections-pl-2 sections-pb-1"
                        style="font-size: 10px"
                      >
                        {{ $t('by') + type.application }}
                      </div>
                      <div
                        v-if="type.app_status === 'disbaled' || type.app_status === 'disabled'"
                        class="section-delete"
                      >
                        <div
                          class="section-delete-icon"
                          @click="
                            openAuthConfigurableSectionTypeModal(
                              type.application_id,
                              index,
                              type.requirements,
                              type.name,
                      ...
                      <div
                        v-else-if="type.type === 'configurable' || type.type === 'dynamic'"
                        class="section-delete"
                      >
                        <div
                          class="section-delete-icon"
                          @click="
                            openUnAuthConfigurableSectionTypeModal(
                              type.application_id,
                              index,
                              type.name,
                              type.application
                  ...
                  <div
                    v-else-if="
                      !currentSection && (typesTab === 'globalTypes' || isCreateInstance === true)
                    "
                  ...
                  >
                    <div
                      v-if="
                        ...
                        globalTypes.filter((gt) => gt.id !== undefined).length === 0 &&
                        loading === false
                      "
                    >
                    ...
                    <div
                      class="section-item section-item-box"
                      v-for="(type, index) in isCreateInstance === true
                        ? filteredGlobalTypes.filter((gt) => gt.notCreated === true)
                        : filteredGlobalTypes.filter((gt) => gt.id !== undefined)"
                      :key="`${type.name}-${index}`"
                    >
                      <div
                        v-if="
                          type.type === 'local' ||
                          componentsSetupData(
                            type && type.section ? type.section.name : type.name,
                            type.type
                          ).settings ||
                          componentsSetupData(
                            type && type.section ? type.section.name : type.name,
                      ...
                      <div v-if="type.notCreated !== true" class="section-delete">
                        <div
                          class="section-delete-icon"
                          @click="openDeleteSectionTypeModal(type.name, index)"
                        >
                          <LazyBaseIconsTrash class="trash-icon-style" />
                        </div>
                      </div>
                      <div
                        v-if="type.query_string_keys && type.query_string_keys.length > 0"
                        class="global-section-info"
                      >
                        <LazyTooltipClickableTooltip
                          :content="`query_string(s): ${type.query_string_keys.join(', ')}`"
                          position="top"
                      ...
                      <div
                        v-else-if="isCreateInstance === true"
                        class="section-top-separator"
                      ></div>
                      <div
                        class="section-item"
                  ...
                  <div
                    v-else
                    :ref="
                      currentSection.name === 'SimpleCTA'
                        ? 'intro-simple-CTA-section-form'
                        : undefined
                    "
                    :class="
                      currentSection.name === 'SimpleCTA' ? 'intro-simple-CTA-section-form' : ''
                    "
                    class="flexSections component-view-wrapper"
                  >
                    <div class="component-view">
                      ...
                      <LazyBaseTypesStatic
                        ...
                        @addSectionType="
                          (section) =>
                            currentSection.instance === true
                              ? currentSection.linked_to !== '' &&
                                currentSection.linked_to !== undefined
                                ? updateGlobalType(section)
                                : addNewGlobalType(section)
                              : addSectionType(section)
                        "
                      ...
                      <LazyBaseTypesConfigurable
                        ...
                        :base-path="pagePath"
                        @loadReference="
                          sectionsConfigurableTypeReference.value = sectionsConfigurableType
                        "
                        @load="(value) => (loading = value)"
                        @promote-section="currentSection = { ...currentSection, instance: true }"
                      />
            ...
            <div
              v-if="isDeleteModalOpen && admin && editMode"
              ref="modal"
              ...
              >
                ...
                >
                  <div class="sections-text-center h4 sections-my-3 sections-pb-3">
                    {{
                      showMyGlobal && deletingGlobalInstance
                        ? $t('delete-global-section-type') + selectedSectionTypeName
                  ...
                  <div class="flexSections sections-flex-row">
                    ...
                    <button class="hp-button" @click="isDeleteModalOpen = false">
                      <div class="btn-text">
                        {{ $t('Cancel') }}
                      </div>
                    </button>
            ...
            <div
              v-if="isRestoreSectionOpen && admin && editMode"
              ref="modal"
              class="sections-fixed sections-z-200 sections-overflow-hidden bg-grey sections-bg-opacity-25 sections-inset-0 sections-p-8 sections-overflow-y-auto modalContainer"
              aria-labelledby="modal-title"
              role="dialog"
              ...
              >
                ...
                >
                  ...
                  <div class="flexSections sections-flex-row">
                    ...
                    <button class="hp-button" @click="isRestoreSectionOpen = false">
                      <div class="btn-text">
                        {{ $t('Cancel') }}
                      </div>
                    </button>
            ...
            <div
              v-if="isDeletePageModalOpen && admin && editMode"
              ref="modal"
              class="sections-fixed sections-z-200 sections-overflow-hidden bg-grey sections-bg-opacity-25 sections-inset-0 sections-p-8 sections-overflow-y-auto modalContainer"
              aria-labelledby="modal-title"
              role="dialog"
            ...
            >
              ...
              >
                ...
                >
                  <div
                    class="flexSections sections-flex-row sections-justify-center sections-items-center"
                  >
                    <LazyBaseIconsAlert />
                    <div
                      class="sections-text-center h4 sections-my-3 sections-pb-3 deletePageLabel"
                    >
                      {{ $t('deletePage') }}
                    </div>
                  </div>
                  <div
                    class="sections-text-center h4 sections-my-3 sections-pb-3 deletePageConfirmation"
                  >
                  ...
                  <div class="flexSections sections-flex-row sections-justify-center">
                    ...
                    <button class="hp-button" @click="isDeletePageModalOpen = false">
                      <div class="btn-text">
                        {{ $t('Cancel') }}
                      </div>
                    </button>
            ...
            <!-- This is delete section page popup that opens when the admin click on the delete page button in red located at the top bottom of the page -->
            <div
              v-if="isDeleteSectionModalOpen && admin && editMode"
              ref="modal"
            ...
            >
              ...
              >
                <div
                  class="section-modal-content bg-white relativeSections shadow rounded-xl overflow-scroll"
                >
                  ...
                  <div class="flexSections flex-row justify-center">
                    <button class="hp-button danger" @click="deleteView(deletedSectionId)">
                      <div class="btn-text">
                        {{ $t('Confirm') }}
                      </div>
                    </button>
                    <button class="hp-button" @click="isDeleteSectionModalOpen = false">
                      <div class="btn-text">
                        {{ $t('Cancel') }}
                      </div>
                    </button>
            ...
            <div
              v-if="isErrorsFormatModalOpen && admin && editMode"
              ref="modal"
            ...
            <div
              v-if="isAuthModalOpen && admin && editMode"
              ref="modal"
              class="sections-fixed sections-z-200 sections-overflow-hidden bg-grey sections-bg-opacity-25 sections-inset-0 sections-p-8 sections-overflow-y-auto modalContainer"
              aria-labelledby="modal-title"
              role="dialog"
                  ...
                  <div class="flexSections sections-flex-col sections-gap-4">
                    ...
                    <div class="flexSections sections-flex-row">
                      <button
                        class="hp-button"
                        @click="
                          authorizeSectionType(selectedSectionTypeAppId, selectedSectionTypeIndex)
                        "
                      >
                        <div class="btn-text">
                          {{ $t('Confirm') }}
            ...
            <div
              v-if="isUnAuthModalOpen && admin && editMode"
              ref="modal"
              class="sections-fixed sections-z-200 sections-overflow-hidden bg-grey sections-bg-opacity-25 sections-inset-0 sections-p-8 sections-overflow-y-auto modalContainer"
              aria-labelledby="modal-title"
              role="dialog"
                  ...
                  <div class="flexSections sections-flex-col sections-gap-4">
                    <div class="flexSections sections-flex-row">
                      <button
                        class="hp-button"
                        @click="
                          unAuthorizeSectionType(selectedSectionTypeAppId, selectedSectionTypeIndex)
                        "
                      >
                        <div class="btn-text">
                          {{ $t('Confirm') }}
                      ...
                      <button class="hp-button" @click="isUnAuthModalOpen = false">
                        <div class="btn-text">
                          {{ $t('Cancel') }}
                        </div>
                      </button>
            ...
            <div
              v-if="isCreatePageModalOpen && admin && editMode"
              ...
              >
                ...
                >
                  ...
                  <div class="flexSections sections-flex-col">
                    ...
                    />
                    <span class="pagePathRequiredStyle" v-show="createPageNameError !== ''">{{
                      createPageNameError
                    }}</span>
                    ...
                    />
                    <span class="pagePathRequiredStyle" v-show="createPagePathError !== ''">{{
                      createPagePathError
                    }}</span>
            ...
            <div v-if="errorInLayout === true && admin && editMode" class="views">
              <div class="flexSections not-found-error">
                <div class="flexSections not-found-error-column">
                  ...
                  <div
                    v-for="(error, index) in sectionsMainErrors"
                    :key="`layout-error-${index}`"
                    class="mainmsg not-found-error-column"
                  >
                    {{ error }}
                  </div>
                  <div
                    v-for="(layoutError, layoutIndex) in sectionsLayoutErrors"
                    :key="`layout-region-error-${layoutIndex}`"
                    class="mainmsg not-found-error-column"
                  >
                    {{ layoutError }}
                  </div>
            ...
            <div v-else-if="selectedLayout === 'standard'" class="views views-content-wrapper">
                    ...
                    <div class="section-view relativeSections">
                      <div
                        ...
                        v-if="
                          admin &&
                          editMode &&
                          sectionOptions[view.id] &&
                          sectionOptions[view.id] === true &&
                          view.altered !== true
                        "
                      >
                        <div
                        ...
                        <div v-if="seoSectionsSupport[view.name]" @click="seoBtnClicked(view.id)">
                          <div
                            :title="
                              pageMetadata.seo && pageMetadata.seo[view.id] === true
                                ? $t('seoDisable')
                                : $t('seoEnable')
                            "
                            class="seo-btn"
                            :class="{
                              enabled: pageMetadata.seo && pageMetadata.seo[view.id] === true,
                            }"
                          >
                            SEO
                          </div>
                      ...
                      <div
                        ...
                        :title="
                          view.linked_to !== '' && view.linked_to !== undefined
                            ...
                            : `${view.name} (${view.id})`
                        "
                        @click="toggleSectionsOptions(view.id)"
                        class="controls optionsSettings sections-flex-row sections-justify-center sections-p-1 rounded-xl sections-top-0 sections-right-2 sections-absolute settings-icon-wrapper sections-cursor-pointer"
                        :class="{ flexSections: !isSideBarOpen }"
                      >
                      ...
                      <div
                        ...
                        :class="
                          admin &&
                          editMode &&
                          invalidSectionsErrors[`${view.name}-${view.weight}`] &&
                          invalidSectionsErrors[view.name].error &&
                          invalidSectionsErrors[`${view.name}-${view.weight}`].weight ===
                            view.weight
                            ? 'invalidSection'
                            : ''
                        "
                      ...
                      >
                        <div
                          v-if="
                            ...
                            editMode &&
                            invalidSectionsErrors[`${view.name}-${view.weight}`] &&
                            invalidSectionsErrors[`${view.name}-${view.weight}`].error &&
                            invalidSectionsErrors[`${view.name}-${view.weight}`].weight ===
                              view.weight
                          "
              ...
              <section
                v-if="creationView === true && admin && editMode && selectedLayout === 'standard'"
                :id="`${currentSection.name}-${currentSection.id}`"
                :section-id="currentSection.id"
                class="creation-view-standard"
              >
                <component
                  :is="getCreationComponent"
                  :section="createdView"
                  :lang="lang"
                  :locales="locales"
                  :default-lang="defaultLang"
                  ref="creationComponent"
                />
              </section>
            </div>
            <div v-else>
              <component
                :is="getSelectedLayout()"
                :lang="lang"
                :locales="locales"
                :default-lang="defaultLang"
                :admin="admin"
                ...
                <template v-for="(slotName, slotIdx) in layoutSlotNames" v-slot:[slotName]>
                  ...
                  <div class="flexSections flex-col">
                      ...
                      <div class="relativeSections">
                        <div
                          ...
                          v-if="
                            admin &&
                            editMode &&
                            regionsOptions[slotName] &&
                            regionsOptions[slotName] === true &&
                        ...
                        <div
                          ...
                          class="controls region regionOptionsSettings sections-flex-row sections-justify-center sections-p-1 rounded-xl sections-top-0 sections-right-2 sections-absolute settings-icon-wrapper sections-cursor-pointer z-50"
                          :class="{ flexSections: !isSideBarOpen }"
                        >
                          <LazyBaseIconsSettings :color="'currentColor'" class="settings-icon" />
                        </div>
                      ...
                      <draggable
                        ...
                        :class="{
                          'highlighted-regions-plus':
                            alteredViewsPerRegions[slotName].length === 0 && highlightRegions,
                        }"
                      >
                        <!-- <transition-group> -->
                        <template #item="{ element: view, index }">
                          <section
                            ...
                            :class="{
                              [view.name]: true,
                              'view-in-edit-mode': editMode,
                              'highlited-regions': highlightRegions,
                            }"
                          >
                            <div class="section-view relativeSections">
                              <div
                    ...
                    <section
                      ...
                      :id="`${currentSection.name}-${currentSection.id}`"
                      :section-id="currentSection.id"
                      :class="`creation-view-${selectedLayout}-${slotName}`"
                    >
                      <component
                        :is="getCreationComponent"
                        :section="createdView"
            ...
            <div
              v-if="staticModal && admin && editMode"
                      ...
                      <div class="body" style="text-align: start">
                        ...
                        >
                          <div class="flexSections">
                            ...
                            />
                            <span
                              class="flexSections sections-flex-row sections-pl-2 sections-items-center"
                            >
                              <span
                                v-show="k || (!k && fieldsInputs.length > 1)"
                                class="sections-cursor-pointer sections-text-3xl"
                                @click="removeField(k)"
                                >-</span
                              >
                              <span
                                v-show="k === fieldsInputs.length - 1"
            ...
            <div
              v-if="staticSuccess && admin && editMode"
              :modal-class="'section-modal-main-wrapper'"
              ref="modal"
              class="sections-fixed sections-z-200 sections-overflow-hidden bg-grey sections-bg-opacity-25 sections-inset-0 sections-p-8 sections-overflow-y-auto modalContainer"
              aria-labelledby="modal-title"
              ...
              >
                ...
                >
                  <div class="section-modal-wrapper success-section-type">
                      ...
                      <div class="title">
                        {{
                          typesTab === 'types' || typesTab === 'inventoryTypes'
                            ...
                            : $t('success-global-section-title')
                        }}
                      </div>
                    ...
                    <div
                      v-if="typesTab === 'types' || typesTab === 'inventoryTypes'"
                      class="flexSections sections-w-full sections-justify-center"
                    ></div>
                    <div class="footer">
          ...
          <div v-else>
            ...
            <button
              ref="intro-create-page"
              v-if="admin && errorResponseStatus !== 401"
              class="intro-create-page hp-button btn-text"
              @click="createNewPage"
            >
              {{ $t('Create New Page') }}
            </button>
            <div
              v-if="
                (errorResponseStatus === 404 || errorResponseStatus === 401) &&
                (errorRegisteredPage === 'page_not_found' ||
                  errorRegisteredPage === 'project_not_found')
              "
            >
              <component
                :is="
                  registeredPage(
                    errorResponseStatus === 404 ? 'page_not_found' : 'project_not_found'
                  )
                "
                :error-response="errorResponseData"
                :error-response-status="errorResponseStatus"
              />
            </div>
...
import {
  ...
  getUser,
  requestVerification,
} from '../../utils/SectionsCMSBridge/functions.js'
...
const updatedPageSettingsTabs = computed(() => {
  ...
  if (admin && editMode.value && builderSettingsTabs && builderSettingsTabs.length > 0) {
    return [...settingsTabs.value, ...builderSettingsTabs]
  } else {
    return [...settingsTabs.value]
  }
})
...
const pageHasNoChanges = computed(() => {
  if (
    ...
    originalVariations.value[selectedVariation.value]?.views
  ) {
    return (
      isEqual(
        Object.values(displayVariations.value[selectedVariation.value].views).map((view) => {
          return {
            ...view,
            fields: undefined,
            multiple: undefined,
...
const currentViews = computed({
  get() {
    let views = []
    if (
      displayVariations.value[selectedVariation.value] &&
      displayVariations.value[selectedVariation.value].views
    ) {
      views = Object.values(displayVariations.value[selectedVariation.value].views)
...
const alteredViewsPerRegions = computed({
  get: () => {
    ...
    if (
      hooksJs &&
      hooksJs['page_pre_render'] &&
      pageData.value &&
      viewsPerRegions.value &&
      Object.keys(viewsPerRegions.value).length > 0
    ) {
      if (typeof hooksJs['page_pre_render'] === 'function') {
        alteredSections = hooksJs['page_pre_render'](
          JSON.parse(JSON.stringify(pageData.value)),
...
const filteredGlobalTypes = computed(() => {
  ...
  globalTypes.value.forEach((type) => {
    getComponentSetup(type?.section ? type.section.name : type.name, type?.type ?? 'static')
  })
...
useHead(() => {
  const baseURL = process.server
    ? (nuxtApp.ssrContext.event.req.headers['x-forwarded-proto'] || 'http') +
      '://' +
      nuxtApp.ssrContext.event.req?.headers.host
  ...
  return {
    ...
    meta: [
      ...
      computedSEO.value.image
        ? {
            hid: 'og:image',
            property: 'og:image',
            content: computedSEO.value.image.url
              ? computedSEO.value.image.url
              : computedSEO.value.image,
          }
        : {},
    ...
    link: [
      projectMetadata.value['selectedCSSPreset'] &&
      projectMetadata.value['selectedCSSPreset'].name &&
      projectMetadata.value['selectedCSSPreset'].name !== 'Other' &&
      projectMetadata.value['selectedCSSPreset'].name !== 'None'
        ? {
            rel: 'stylesheet',
        ...
        : projectMetadata.value['selectedCSSPreset'] &&
            projectMetadata.value['selectedCSSPreset'].name &&
            projectMetadata.value['selectedCSSPreset'].name !== 'None' &&
            projectMetadata.value['media'] &&
            projectMetadata.value['media'].url
          ? {
              rel: 'stylesheet',
...
const initializeSectionsCMSEvents = () => {
  if (!window.SectionsCMS) {
    ...
    if (admin) {
      ...
      window.SectionsCMS.value.runIntro = (topic, rerun, lastSavedTopic) =>
        runIntro(topic, rerun, lastSavedTopic)
      window.SectionsCMS.value.addNewStaticType = (name, payload) =>
        addNewStaticType(name, payload, true)
      window.SectionsCMS.value.addNewGlobalType = (instance_name, payload) =>
        addNewGlobalType(null, instance_name, payload, true)
      window.SectionsCMS.value.updateProjectMetadata = (payload) =>
        updateProjectMetadata(payload, true)
      window.SectionsCMS.value.createNewPage = (page_name, payload) =>
        createNewPage(page_name, payload, true)
...
const initializeSections = (res, skipHook) => {
  ...
  if (
    ...
    projectMetadata.value['languages'] &&
    projectMetadata.value['languages'].length > 0
  ) {
  ...
  if (
    ...
    res.data.metadata.project_metadata.activateCookieControl !== undefined &&
    res.data.metadata.project_metadata.activateCookieControl !== null
  ) {
    projectMetadata.value.activateCookieControl =
      res.data.metadata.project_metadata.activateCookieControl
  }
  if (
    res.data.metadata.project_metadata &&
    res.data.metadata.project_metadata.gtmId !== undefined &&
    res.data.metadata.project_metadata.gtmId !== null
  ) {
  ...
  if (res.data.metadata && res.data.metadata.project_metadata) {
    ...
    try {
      ...
      if (builderHooksJavascript['initialize_builder_settings']) {
        builderHooksJavascript['initialize_builder_settings'](
          originalBuilderSettings.value,
          useHead,
  ...
  sections.map((section) => {
    ...
    if (section.type === 'configurable') {
      if (
        section.render_data &&
        section.render_data[0] &&
        section.render_data[0].settings &&
        section.render_data[0].settings.image &&
        !Array.isArray(section.render_data[0].settings.image)
      ) {
        section.render_data[0].settings.image = []
      }
      ...
      section.settings = isJsonString(section.settings)
  ...
  if (!originalVariations.value[activeVariation.value.pageName]) {
    originalVariations.value = JSON.parse(JSON.stringify(displayVariations.value))
  }
  ...
  if (
    hooksJs &&
    hooksJs['section_page_initialization_completed'] &&
    hooksJs['section_page_initialization_completed'](res, i18n, useCookie, route)
  ) {
...
const sectionsPageErrorManagement = (error, server, skipHook) => {
  ...
  if (
    ...
    error.response.data.options.project_metadata.pagePath404 &&
    error.response.data.options.project_metadata.pagePath404 !== '' &&
    error.response.data.options.project_metadata.pagePath404 !== pagePath &&
    !useCookie('sections-auth-token').value
  ) {
  ...
  if (
    (errorResponseStatus.value === 404 || errorResponseStatus.value === 401) &&
    registeredPage(errorResponseStatus.value === 404 ? 'page_not_found' : 'project_not_found')
  ) {
    errorRegisteredPage.value =
      errorResponseStatus.value === 404 ? 'page_not_found' : 'project_not_found'
  ...
  if (tab === 'page_settings') {
    ...
    try {
      ...
      if (builderHooksJavascript['handle_unsaved_settings']) {
        unsavedSettingsError.value[tab] = builderHooksJavascript['handle_unsaved_settings'](
          isEqual,
          originalBuilderSettings.value,
          updatedBuilderSettingsPerTab.value[tab],
          tab
        )
      } else {
        unsavedSettingsError.value[tab] = false
...
const builderSettingUpdated = (settings) => {
  ...
  try {
    ...
    if (builderHooksJavascript['builder_settings_payload']) {
      builderSettingsPayload.value = builderHooksJavascript['builder_settings_payload'](
        originalBuilderSettings.value,
        settings,
        currentSettingsTab.value
      )
    ...
    const res = await useApiRequest({
      ...
      onSuccess: (res) => {
        ...
        if (external_call !== true) {
          ...
          showToast('Success', 'success', i18n.t('successSettingsChanges'))
        }
      },
      onError: (error) => {
        loading.value = false
...
const updatePageMetaData = async (seo, themeData) => {
  ...
  if (updatedPagePath !== '/') {
    ...
    const uniquePathSegments = pathSegments.filter(
      (segment, index) => segment !== '' && segment !== pathSegments[index - 1]
    )

    // Reconstruct the URL with the unique path segments
    updatedPagePath = updatedPagePath.endsWith('/')
      ? '/' + uniquePathSegments.join('/') + '/'
      : '/' + uniquePathSegments.join('/')
  ...
  if (!validation.valid) {
    ...
    showToast('Invalid page path', 'error', validation.error)
    return
  }
    ...
    await useApiRequest({
      ...
      onSuccess: (res) => {
        ...
        try {
          ...
          if (
            hooksJs &&
            hooksJs['reload_page_on_path_update'] &&
            hooksJs['reload_page_on_path_update'](useCookie)
          ) {
...
const exportSections = () => {
  const dataStr =
    ...
    encodeURIComponent(
      JSON.stringify({ layout: selectedLayout.value, sections: allSections.value })
    )
...
const initImportSections = () => {
  if (Object.keys(displayVariations.value[selectedVariation.value].views).length > 0) {
    showToast('Warning', 'warning', i18n.t('importSections'))
  } else {
    // In Nuxt 3, we use template refs differently
  ...
  reader.onload = (evt) => {
    ...
    sections.forEach((section) => {
      ...
      if (
        ...
        section.region[selectedLayout.value].slot
      ) {
  ...
  if (sectionTypeName.value !== '') {
    if (selectedLayout.value !== 'standard') {
      ...
      section.region[selectedLayout.value] = {
        ...
        weight: Object.keys(displayVariations.value[selectedVariation.value].views).length,
      }
    } else {
      section.region = {}
    }
      ...
      await useApiRequest({
        ...
        onError: (error) => {
          ...
          showToast(
            'Error',
            'error',
            i18n.t('updateSectionTypeError') + error.response.data.message,
            error.response.data.options
          )
        },
      })
        ...
        onSuccess: async () => {
          if (external_call !== true) {
            ...
            if (canPromote.value === true) {
              ...
              section.region =
                displayVariations.value[selectedVariation.value].views[section.id].region
              ...
              showToast('Success', 'info', i18n.t('successAddedSection'))
            }
        ...
        onError: (error) => {
          ...
          if (external_call !== true) {
            showToast(
              'Error',
              'error',
              i18n.t('createSectionTypeError') + error.response.data.message,
              error.response.data.options
            )
          }
        },
...
const addNewStaticType = async (name, payload, external_call, createSection) => {
  ...
  if (sectionTypeName.value !== '') {
    ...
    if (name && external_call !== true) {
      const formComp = await importComp(`/forms/${sectionTypeName.value}`).setup?.then(
        (d) => d.default
      )
    ...
    fieldsDeclaration =
      external_call === true && payload.fields
        ? payload.fields
        : fieldsDeclaration.filter((field) => field.name.trim() !== '')
    ...
    try {
      const res = await useApiRequest({
        ...
        onError: async (error) => {
          // Make onError async if getSectionTypes is called
          loading.value = false
          if (external_call !== true) {
            types.value = []
            globalTypes.value = []
...
const getComponentSetup = async (sectionName, sectionType) => {
  ...
  if (
    moduleData &&
    moduleData.props &&
    moduleData.props.viewStructure &&
    ...
    const setupData = {
      ...
      render_data: populateWithDummyValues(
        moduleData.props.viewStructure.render_data,
        dummyDataPresets
      ),
      type: sectionType,
    }
    ...
    if (view && pageMetadata.value.seo && pageMetadata.value.seo[view.id] === true) {
      if (sectionHooksJs && sectionHooksJs['seo_management']) {
        const seo = JSON.parse(
          JSON.stringify(sectionHooksJs['seo_management'](view, i18n.locale.value))
        )
...
const getComponent = (sectionName, sectionType, view) => {
  ...
  if (
    hooksJs &&
    hooksJs['section_pre_render'] &&
    hooksJs['section_pre_render']({ sectionName, sectionType })
  ) {
...
const computeLayoutData = async () => {
  ...
  if (selectedLayout.value !== 'standard') {
    ...
    if (!layoutComp?.props) {
      ...
      !layoutComp.props.slotNames.type ||
      layoutComp.props.slotNames.type !== Array ||
      !layoutComp.props.slotNames.default
    ) {
      errorInLayout.value = true
  ...
  setTimeout(() => {
    nextTick(() => {
      if (selectedLayout.value !== 'standard') {
        ...
        layoutSlotNames.value.forEach((slotName) => {
          if (
            !document.getElementById(`sections-slot-region-${selectedLayout.value}-${slotName}`)
          ) {
            errorInLayout.value = true
            sectionsLayoutErrors.value.push(
              slotName.charAt(0).toUpperCase() +
                slotName.slice(1) +
                ' ' +
                i18n.t('layoutErrors.regionNotConfigured')
            )
            sectionsLayoutErrors.value.push(
              `<slot name=\"${slotName}\"></slot> ${i18n.t('layoutErrors.layoutTemp')}`
            )
            return
          }
        })
...
const sanitizeWeightsByInsertion = (
  views,
  layoutKey,
  targetSlot,
  priorityViewId,
  ...
  const otherViews = Object.entries(views)
    .filter(([id, view]) => view.region?.[layoutKey]?.slot === targetSlot && id !== priorityViewId)
    .map(([id, view]) => ({ id, view }))
...
const logDrag = (evt, slotName) => {
  if (evt.added) {
    ...
    displayVariations.value[selectedVariation.value].views[viewId].region[
    ...
    const movedSlot =
      displayVariations.value[selectedVariation.value].views[viewId].region[selectedLayout.value]
        .slot
...
const createNewPage = async (page_name, payload, external_call, custom) => {
  ...
  let updatedPagePath =
    createPagePath.value && createPagePath.value !== '' ? createPagePath.value.trim() : ''
  ...
  try {
    const res = await useApiRequest({
      ...
      onSuccess: (res) => {
        loading.value = false
        if (custom) {
          showToast('Success', 'success', i18n.t('createPageSuccess'))
          myPages.value.push({
...
const runIntro = async (topic, rerun, lastSavedTopic, action) => {
  ...
  if (
    guideConfig.value.autoStart === false &&
    action !== 'global-content-guide-btn' &&
    action !== 'relaunch-guide-btn'
  ) {
    return
  } else if (
    guideConfig.value.autoStart === false &&
    (action === 'global-content-guide-btn' || action === 'relaunch-guide-btn')
  ) {
  ...
  setTimeout(() => {
    ...
    function handleSkipClick() {
      ...
      introRerun.value = false
      if (
        useCookie('intro-last-step').value &&
        useCookie('intro-last-step').value === 'addNewSectionModal'
      ) {
        useCookie('intro-last-step').value = null
      }
    }
    ...
    await useApiRequest({
      ...
      onError: (e) => {
        ...
        if (e.response.status === 404) {
          ...
          addSectionType({
            // Changed from emit('addSectionType', ...) to direct call
            name: gt.section.name,
            type: 'configurable',
            settings: options[0],
            private_data: gt.section.private_data ?? {},
          ...
          errorAddingSection({
            // Changed from emit('errorAddingSection', ...)
            closeModal: false,
            title: 'Error adding ' + gt.name,
            message: e.response.data.error
              ? e.response.data.error
              : i18n.t('saveConfigSectionError'),
          })
        }
      },
    })
      ...
      onSuccess: async (res) => {
        ...
        res.data.data.forEach((d) => {
          ...
          globalTypes.value.push({
            ...
            query_string_keys: types.value.find((t) => t.name === d.section.name)
              ?.query_string_keys,
                ...
                addSectionType(
                  {
                    ...
                    query_string_keys: gt.query_string_keys,
                    dynamic_options: gt.dynamic_options,
                    render_data: gt.section?.options?.[0]
                      ? [{ settings: gt.section.options[0] }]
    ...
    await useApiRequest({
      ...
      onSuccess: async (res) => {
        ...
        availableSectionsForms.value.forEach((name) => {
          const found = types.value.find((element) =>
            element.name.includes(':') ? element.name.split(':')[1] === name : element.name === name
          )
          if (!found) {
            types.value.push({
              name,
...
const openEditMode = async () => {
  ...
  if (editMode.value === true) {
    ...
    const config = {
      headers: sectionHeader(
        inBrowser ? { token } : { origin: nuxtApp.$sections.projectUrl, token }
      ),
    }
    ...
    if (
      nuxtApp.$sections.queryStringSupport &&
      nuxtApp.$sections.queryStringSupport === 'enabled'
    ) {
      ...
      await useApiRequest({
        ...
        onSuccess: async (res) => {
          ...
          if (res.data.last_updated > sectionsPageLastUpdated.value) {
            showToast('Warning', 'warning', i18n.t('oldPageVersion'))
          }
          initializeSections(res)
...
const synch = () => {
  ...
  const currentVariationView = displayVariations.value[selectedVariation.value].views

  // remove all existing linked to
  const withoutLinkedToValueList = Object.values(currentVariationView).filter(
    (view) => !view.linkedTo
  )
...
const addSectionType = (section, showToastBool = true, instance = false) => {
  try {
    if (savedView.value.linkedTo) {
      const confirmed = window.confirm(i18n.t('linkedSection'))
      if (!confirmed) {
        return
      }
    }
    ...
    if (section.weight === null || section.weight === 'null' || section.weight === undefined) {
      section.weight = Object.keys(displayVariations.value[selectedVariation.value].views).length
    }

    if (selectedLayout.value !== 'standard') {
      if (
        section.region === undefined ||
        section.region === null ||
        section.region[selectedLayout.value] === undefined ||
        section.region[selectedLayout.value] === null
      ) {
        section.region = {}
      }
      section.region[selectedLayout.value] = {
        ...
        weight:
          section.region &&
          section.region[selectedLayout.value] &&
          section.region[selectedLayout.value].weight !== undefined &&
          section.region[selectedLayout.value].weight !== null
            ? section.region[selectedLayout.value].weight
            : viewsPerRegions.value[selectedSlotRegion.value]
              ? viewsPerRegions.value[selectedSlotRegion.value].length
              : Object.keys(displayVariations.value[selectedVariation.value].views).length,
      }
    }

    if (
      instance === true ||
      (section.type === 'local' && section.instance_name) ||
      (section.type === 'dynamic' && section.instance_name) ||
      (section.type === 'configurable' && section.instance_name)
    ) {
      section.linkedTo = section.instance_name
      section.linked_to = section.instance_name
      section.instance = true
      section.settings =
        (section.type === 'dynamic' || section.type === 'configurable') &&
        section.render_data &&
        section.render_data[0] &&
        section.render_data[0].settings
          ? section.render_data[0].settings
    ...
    if (selectedVariation.value === pageName) {
      ...
      variations.map((variation) => {
        const newViews = Object.values(displayVariations.value[variation.pageName].views).map(
          (sectionVariation) => {
    ...
    showToast('Error', 'error', i18n.t('previewSectionError'))
  }
}
const refreshSectionView = async (sectionView, data) => {
  let sectionDatas = []
  const reRenderMultipleSections =
  ...
  if (reRenderMultipleSections === true) {
    ...
    sectionDatas.map((section) => {
      ...
      section.renderOptions =
        sectionTargetByID && sectionTargetByID.options ? sectionTargetByID.options : null
      section.renderID = sectionTargetByID && sectionTargetByID.id ? sectionTargetByID.id : null
      return section
    })
  } else {
    sectionDatas = allSections.value.filter(
      (section) =>
        (section.query_string_keys &&
          section.query_string_keys.length > 0 &&
          Object.keys(data.qs).some((qsItem) => section.query_string_keys.includes(qsItem))) ||
        (data.qs.language && sectionsQueryStringLanguageSupport.value.includes(section.name))
    )
  }
            ...
            if (res.data && res.data.error) {
              ...
              if (sectionData.type === 'configurable' && sectionData.renderID) {
                currentViews.value = currentViews.value.map((view) => {
                  if (
                    view.type === 'configurable' &&
                    sectionData.renderID &&
                    sectionData.renderID === view.id
                  ) {
                    return {
                      ...view,
                      render_data: res.data.render_data,
      ...
      if (!view.error || view.status_code === 404) {
        ...
        if (view.settings && view.type === 'configurable') {
          ...
          view.render_data.map((rData) => {
            if (
              (rData.private_data &&
                Object.keys(rData.private_data).length > 0 &&
                rData.private_data.image &&
                !Array.isArray(rData.private_data.image)) ||
              (rData.settings.image && !Array.isArray(rData.settings.image))
            ) {
              formatValdiation = false
              showToast('', 'error', i18n.t('imageFieldValidation') + view.name)
              return
            }
            options.push(rData.settings)
          })
  ...
  if (sections.length > 0) {
    function validateData(section, option, field) {
      if (Object.keys(option).includes(field.name)) {
        if (
          option[field.name] &&
          (Array.isArray(option[field.name]) || typeof option[field.name] === 'object') &&
          ...
          if (Array.isArray(option[field.name])) {
            if (
              (field.type === 'image' || field.type === 'media') &&
              (!option[field.name][0] ||
                !option[field.name][0].media_id ||
                !option[field.name][0].url) &&
              option[field.name].length !== 0
            ) {
              ...
              sectionsFormatErrors[section.weight] =
                `${i18n.t('wrongFieldName')} \`${field.name}\` ${i18n.t('formatOfSection')} \`${section.name}\``
            }
          } else if (typeof option[field.name] === 'object') {
            if (
              !option[field.name].media_id ||
              !option[field.name].url ||
              Object.keys(option[field.name]).length === 0
            ) {
              integrityCheck = false
              loading.value = false
              showToast(
        ...
        await useApiRequest({
          ...
          onSuccess: async (res) => {
            ...
            if (res.data.invalid_sections && res.data.invalid_sections.length > 0) {
              showToast('Error', 'error', i18n.t('someSectionsNotSaved'))
              ...
              showToast('Success', 'success', i18n.t('successPageChanges'))
...
const restoreVariations = () => {
  displayVariations.value = JSON.parse(JSON.stringify(originalVariations.value))
  selectedLayout.value = sectionsLayout.value
  computeLayoutData()
  showToast('Revert Successful', 'info', i18n.t('revertPageSuccess'))
}
...
const deleteView = (id) => {
  ...
  try {
    ...
    if (builderHooksJavascript['section_removed']) {
      const updatedMetaData = builderHooksJavascript['section_removed'](
  ...
  showToast('Deleted', 'info', i18n.t('sectionRemoved'))
  computeLayoutData()
}
  ...
  try {
    await useApiRequest({
      ...
      onSuccess: async (res) => {
        // Make onSuccess async
        if (res.data) {
          showToast('Success', 'info', res.data.message)
...
const authorizeSectionType = async (sectionAppId, index) => {
  ...
  try {
    await useApiRequest({
      ...
      onSuccess: (res) => {
        if (res.data) {
          // Check if res.data exists, though success implies it should
          showToast(
            'Success',
            'info',
          ...
          types.value
            .filter((type) => type.application_id === sectionAppId)
            .forEach((type) => {
      ...
      onError: (error) => {
        showToast(
          'Error',
          'error',
          `${i18n.t('authorizeError')} ${selectedAppName.value}: ` + error.response.data.message
        )
        loading.value = false
        emit('load', false)
      },
    ...
    await useApiRequest({
      ...
      onError: (error) => {
        showToast(
          'Error',
          'error',
          `${i18n.t('unAuthorizeError')} ${selectedAppName.value}: ` + error.response.data.message
        )
        loading.value = false
        emit('load', false)
      },
...
const openAuthConfigurableSectionTypeModal = (
  sectionAppId,
  index,
  requirements,
  sectionTypeName,
...
const openUnAuthConfigurableSectionTypeModal = (
  sectionAppId,
  index,
  sectionTypeName,
  applicationName
    ...
    currentSection.value = {
      ...
      render_data:
        type.section && type.section.options && type.section.options[0]
          ...
          : undefined,
      addToPage:
        type.type === 'configurable' && isCreateInstance.value === false ? true : undefined,
    }
    ...
    if (type.type === 'static' || type.type === 'configurable' || createSection) {
      ...
      nextTick(() => {
        try {
          if (
            createSection &&
            Array.isArray(filteredTypes.value) &&
...
const settingsTabTitle = (tab) => {
  try {
    const builderHooksJavascript = importJs(`/builder/settings/builder-hooks`)
    if (
      builderHooksJavascript['update_tab_title'] &&
      builderHooksJavascript['update_tab_title'](tab)
    ) {
...
const closeMetadataModal = () => {
  const hasUnsavedSettings = updatedPageSettingsTabs.value.some((tab) => unsavedSettings(tab))
    ...
    updatedPageSettingsTabs.value.forEach((tab) => {
      try {
        ...
        if (builderHooksJavascript['reset_builder_settings']) {
          const settingsReset = builderHooksJavascript['reset_builder_settings'](
            originalBuilderSettings.value,
            tab
          )
    ...
    sectionsThemeComponents.value[currentSectionData.value.section.name].forEach((tab) => {
      try {
        ...
        if (builderHooksJavascript['reset_theme_settings']) {
          const settingsReset = builderHooksJavascript['reset_theme_settings'](
            originalThemeSettings.value,
...
const restoreSection = () => {
  ...
  if (selectedLayout.value !== 'standard') {
    try {
      const index = viewsPerRegions.value[selectedSlotRegion.value].findIndex(
        (view) => view.id === currentSection.value.id
      )
...
const checkIntroLastStep = (topic) => {
  if (
    useCookie('intro-last-step').value &&
    useCookie('introjs-dontShowAgain').value !== true &&
    useCookie('intro-last-step').value === topic
  ) {
...
const closeSectionThemeModal = () => {
  const hasUnsavedSettings = sectionsThemeComponents.value[
    currentSectionData.value.section.name
  ].some((tab) => unsavedThemeSettings(tab.id))
...
const unsavedThemeSettings = (tab) => {
  try {
    const builderHooksJavascript = importJs(`/theme/theme-hooks`)
    if (builderHooksJavascript['handle_unsaved_settings']) {
      unsavedThemeSettingsError.value[tab] = builderHooksJavascript['handle_unsaved_settings'](
        isEqual,
        originalThemeSettings.value,
        currentSectionData.value.currentTheme,
        tab,
        currentSectionData.value.section
      )
    } else {
...
const sectionThemeUpdated = (settings) => {
  try {
    ...
    if (builderHooksJavascript['theme_settings_payload']) {
      themeSettingsPayload.value = builderHooksJavascript['theme_settings_payload'](
        originalThemeSettings.value,
        settings,
        currentThemeTab.value.id,
        currentSectionData.value.section
      )
    }
  } catch {}
  ...
  currentSectionData.value.currentTheme[currentThemeTab.value.id][
    currentSectionData.value.section.id
  ] = settings
}
...
const themeScrollToSection = () => {
  ...
  const id =
    section.linked_to !== '' && section.linked_to !== undefined
...
const fetchData = async () => {
  try {
    ...
    if (hooksJavascript['init_params']) {
      const paramsUpdate = hooksJavascript['init_params'](nuxtApp.$sections, {
        qs: route.query,
        headers:
          nuxtApp.ssrContext && nuxtApp.ssrContext.event.req
            ? nuxtApp.ssrContext.event.req.headers
            : {},
        reqBody:
          nuxtApp.ssrContext && nuxtApp.ssrContext.event.req
            ? nuxtApp.ssrContext.event.req.body
            : {},
        url:
          nuxtApp.ssrContext && nuxtApp.ssrContext.event.req && nuxtApp.ssrContext.event.req.headers
            ? nuxtApp.ssrContext.event.req.headers.host
  ...
  if (
    nuxtApp.$sections.projectLocales &&
    nuxtApp.$sections.projectLocales !== '' &&
    nuxtApp.$sections.projectLocales.includes(',')
  ) {
    translationComponentSupport.value = true
    locales.value = []
  ...
  const config = {
    headers: sectionHeader(
      inBrowser ? { token } : { origin: `${scheme}://${websiteDomain}`, token }
    ),
  }
...
async function loadScriptWithUniqueness(src, uniqueness = true) {
  await loadScript(src, uniqueness, scriptLinksArray, scriptPromises, useScript)
}
...
.section-view .controls {
  ...
  box-shadow:
    var(--tw-ring-offset-shadow, 0 0 rgba(0, 0, 0, 0)), var(--tw-ring-shadow, 0 0 rgba(0, 0, 0, 0)),
    var(--tw-shadow);
  border-width: 1px;
}
...
.section-modal-wrapper .footer button {
  width: auto;
  min-width: auto;
  border-radius: 16px;
  height: auto;
...
.modalContainer .step-back {
  position: absolute;
}

.modalContainer .step-back svg {
  color: #8b8b8b;
}

.modalContainer .step-back svg:hover {
...
.modalContainer .closeIcon svg:hover {
...
.sections-container > .sections-aside {
  ...
  max-width: calc(
    100% - 375px - 3px
  ); /* 375px being the mobile min width and 3px being the width size of the resize handle */
  z-index: 190;
  background: white;
}
...
.sections-aside .closeIcon {
  position: absolute;
  right: 10px;
  cursor: pointer;
}

.sections-aside .closeIcon svg {
  width: 35px;
  height: 35px;
  color: #03b1c7;
}
...
.sections-aside .closeIcon svg:hover {
  color: darken(#03b1c7, 10%);
}

.sections-aside .anchorIcon {
  position: absolute;
  right: 55px;
  top: 16px;
  cursor: pointer;
...
.sections-aside .anchorIcon svg {
  width: 25px;
  height: 25px;
  color: #03b1c7;
}

.sections-aside .anchorIcon svg:hover {
  color: darken(#03b1c7, 10%);
}
...
.sections-theme-settings-tab:last-child {
  border-bottom: none;
}

/* Optional: Add 'active' style with a class like 'active-tab' */
.page-settings-tab.active-tab,
.sections-theme-settings-tab.active-tab {
  background-color: #03b1c7;
  color: white;
  font-weight: bold;
}
.page-settings-tab:first-child.active-tab,
.sections-theme-settings-tab:first-child.active-tab {
  border-top-left-radius: 8px;
  border-top-right-radius: 8px;
}
.page-settings-tab:last-child.active-tab,
.sections-theme-settings-tab:last-child.active-tab {
...
.dynamic-side-component,
.dynamic-side-component-wrapper {
  height: 100%;
}
...
@container sections-main (max-width: 768px) {
  ...
  .modalContainer.prime,
  .inner-modal-conatiner {
    padding: 0;
  }
