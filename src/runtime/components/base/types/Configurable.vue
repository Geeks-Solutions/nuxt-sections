  ...
  <div class="container text-center sub-types" :class="{ containerWidth: !isSideBarOpen }">
    ...
    <div v-show="currentTab === 'config'">
      ...
      <div class="form-group">
        <form>
          <div class="flex flex-col justify-between" :class="{ 'content-wrapper': !isSideBarOpen }">
            ...
            <LazyTranslationsTranslationComponent
              v-if="translationComponentSupport"
              :locales="locales"
              :default-lang="defaultLang"
              @setFormLang="updateLocale"
            />
            ...
            >
              ...
              <div
                v-if="registeredComponents[field.type + '_' + field.key]"
              ...
              <div v-else class="element d-inline-block">
                <div v-if="field.name && field.type !== 'hidden'" class="text-capitalize text-left">
                ...
                <div v-else class="w-full justify-start">
                  ...
                  <div
                    v-if="
                      ...
                      optionsData[field.key].length > 0 &&
                      optionsData[field.key][0].url
                    "
                    class="py-4 flex align-items-center"
                  >
                    <img
                  ...
                  <div
                    v-else-if="field.type === 'media' && previewMedia"
                    class="py-4 flex align-items-center"
                  >
                    <img :src="previewMedia" alt="image" class="w-[95px] h-[63px] object-contain" />
                    <div class="cursor-pointer pl-2" @click="removeImage(field.key)">
                      <LazyBaseIconsClose />
                    </div>
                  </div>
                  ...
                  <div
                    v-else-if="field.type === 'media' && isInProgress"
                    class="loadingCircle pl-4 p-2"
                  >
                  ...
                  >
                    <div class="selectMultipleOptions">
                      <div
                        ...
                        :key="option.id"
                        class="multiple-options-wrapper"
                      >
                        <div
                          class="single-multiple-option"
                          :class="
                            isSelected(option.id, field.name) ? 'multiple-options-selected' : ''
                          "
                          @click="selectOption(option.id, field.name)"
                        >
                  ...
                  <component
                    v-show="
                      ...
                      (!Array.isArray(optionValues.option_values) && field.type !== 'media') ||
                      (field.type === 'media' &&
                        !previewMedia &&
                        (!optionsData[field.key] ||
                          (optionsData[field.key] &&
                    ...
                    :class="
                      optionValues.field === field.name && optionValues.option_values
                        ? 'd-input pl-6 border rounded-xl border-FieldGray w-full focus:outline-none'
                        : field.type !== 'media'
                          ? 'd-input pl-6 border rounded-xl border-FieldGray h-[48px] w-full focus:outline-none'
                          : ''
                    "
                    :id="field.key"
                    :is="
                      getTag(
                        field.type,
                        field.name && field.name.includes(':')
                          ? field.name.split(':')[1]
                          : field.name
                      )
                    "
                  ...
                  >
                    <option
                      ...
                      :selected="
                        ...
                        optionValues.option_values &&
                        optionsData[field.key] &&
                        optionsData[field.key].indexOf(option.id) !== -1
                      "
                    >
                      {{ option.title }}
                    </option>
                ...
                <span v-if="field.type === 'media'" class="flex text-error py-2 text-xs">{{
                  mediaError
                }}</span>
              </div>
            </div>
...
const props = defineProps({
  props: {
    // This holds the nested object with fields like name, fields, linked_to etc.
    type: Object,
    default: () => ({}),
  },
  savedView: {
  ...
  headers: {
    // Likely unused now with Nuxt 3 fetching
    type: Object,
    default: () => ({}),
  },
  html: {
    // Likely related to WYSIWYG initial content
    type: String,
    default: '',
  },
  sectionsUserId: {
  ...
  instance: {
    // Is this a global instance?
    type: Boolean,
    default: false,
  },
  linked: {
    // Is this linked to a global instance?
    type: Boolean,
    default: false,
  },
  basePath: {
...
async function changeFieldValue(e, idx, type, fieldname) {
  ...
  if (type === 'media') {
    ...
    stringType(type) &&
    !(optionValues.field === fieldname && optionValues.option_values)
  ) {
    if (!optionsData[name]) optionsData[name] = {}
    optionsData[name][formLang.value] = value
    if (!options[0][name]) options[0][name] = {} // Legacy
...
function getTag(type, name) {
  ...
  if (
    (type === 'integer' || type === 'string' || type === 'textfield' || type === 'textarea') &&
    optionValues.field === name &&
    optionValues.option_values
  ) {
    return 'select'
  }
...
async function addConfigurable() {
    ...
    if (isFieldRequired(fields)) {
      ...
      if (
        ((typeof options[0][key] === 'string' && !options[0][key]) ||
          (typeof options[0][key] === 'object' &&
            stringType(fields.type) &&
            !options[0][key][props.defaultLang])) &&
  ...
  const variables = {
    section: {
      // Access nested props.props.name
      name:
        props.props.name && props.props.name.includes(':')
          ? props.props.name
          : `${props.savedView.application_id}:${props.props.name}`, // Ensure savedView.application_id is available
  ...
  if (nuxtApp.$sections.queryStringSupport === 'enabled') {
    const query_string = parseQS(
      encodeURIComponent(route.params.pathMatch ? route.params.pathMatch.join('/') : '/'),
  ...
  try {
    ...
    emit('addSectionType', {
      ...
      nameID:
        props.props.name && props.props.name.includes(':')
          ? props.props.name
          : `${props.savedView.application_id}:${props.props.name}`, // Ensure savedView.application_id
      type: 'configurable',
  ...
  try {
    ...
    if (result.res && result.res.data) {
      ...
      showToast(
        'Error',
        'error',
        result.error?.response?.data?.message || 'Failed to load global type data'
      )
    }
  } catch (error) {
