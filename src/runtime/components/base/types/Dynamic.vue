<template>
  <div class="sub-types dynamic-s">
    ...
    <h4 :class="{ 'dynamic-t': !isSideBarOpen }">
      {{
    ...
    <button v-if="globalSectionMode" class="mt-4 submit-btn" type="button" @click="addStatic">
      {{ t('Submit data') }}
    </button>
  </div>
</template>
...
async function renderSection(sectionName) {
  // Use parameter instead of props.name
  emit('load', true)
  const config = {
...
<style scoped>
/* Changed from <style> to <style scoped> */
...
.border-FieldGray {
  /* Copied from Configurable.vue for consistency */
