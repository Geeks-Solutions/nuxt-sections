<template>
  <div class="sub-types">
    <div>
      <div class="text-video" :class="{ 'content-wrapper': isSideBarOpen === false }" v-show="name">
        ...
        <component
          ...
          @openMediaModal="
            (mediaId, category) => sectionsMediaComponentRef?.openModal(mediaId, category)
          "
    ...
    <div class="sections-btns-submission">
      <!-- Submit Button -->
      <button class="mt-4 submit-btn" type="button" @click="sendJsonData" :class="{ withTabs }">
        {{ t('Submit data') }}
      </button>
      <!-- Promote Button -->
      <button
...
function sendJsonData() {
  ...
  if (formComponent.private_data !== undefined) {
    ...
    console.warn(
      `Dynamic form component for "${props.name}" does not have a 'private_data' property.`
    )
    // Potentially try to gather data differently if 'settings' isn't exposed
