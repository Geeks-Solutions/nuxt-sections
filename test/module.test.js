vi.mock('#app', () => ({
  useNuxtApp: () => ({
    $sections: {
      queryStringSupport: 'disabled',
    },
  }),
}))

import { shallowMount, mount } from '@vue/test-utils';
import SectionsMain from '../src/runtime/components/Sections/index.vue';
...
describe('SectionsMain', () => {
  ...
  beforeEach(() => {
    ...
    document.body.appendChild = vi.fn().mockImplementation(() => tooltip);
  });

  test('calls initializeSections and computeLayoutData in fetch()', async () => {
    // Create a spy for both methods
    const initializeSectionsSpy = vi.spyOn(SectionsMain.methods, 'initializeSections')
    const computeLayoutDataSpy = vi.spyOn(SectionsMain.methods, 'computeLayoutData')
  ...
  it('addSectionType function should set section.weight if it is null, "null", or undefined', () => {
    ...
    testCases.forEach((initialWeight, idx) => {
      ...
      const showToast = vi.fn(); // Mock function
  ...
  beforeEach(() => {
    ...
    wrapper = shallowMount(SectionsMain, {
      mocks: {
        ...
        sectionHeader: vi.fn().mockReturnValue({}),
        parseQS: vi.fn((path, hasQuery, query) => ({ path, hasQuery, query })),
        validateQS: vi.fn((queryString, keys, editMode) => ({ validated: true })),
      },
      propsData: {},
    });
  });
...
describe('alteredViews computed property', () => {
  ...
  it('returns alteredSections when page_pre_render is a function and returns a value', () => {
    const mockPagePreRender = vi.fn().mockReturnValue([{ id: 1 }]);
  ...
  it('returns currentViews when page_pre_render returns null', () => {
    const mockPagePreRender = vi.fn().mockReturnValue(null);
    mockImportJs.mockReturnValue({
      page_pre_render: mockPagePreRender,
    });
